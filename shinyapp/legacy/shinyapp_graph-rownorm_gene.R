...
server <- function(input, output) {
  ...
  output$network <- renderPlot({
    ...
    filter_net_edge = filter(net_edge_weight, Weight > input$edge_threshold)
    ...
    for (x in 1:length(all_components)){
      ...
      if (length(each_comp) >= input$giant_comp_threshold){
    ...
    print(paste("Writing edge to:", normalizePath(refilter_edge_path, mustWork = FALSE)))
    print(paste("Writing node to:", normalizePath(refilter_node_path, mustWork = FALSE)))
    tryCatch({
      ...
      print(paste("write.csv ERROR:", e$message))
    })
    ...
    vertex_col[V(net)$t2ds_no_t2ds_pvalue <= input$pvalue_threshold] = '#B2DF8A'   # green: T2D vs No_T2D sig
    vertex_col[V(net)$pret2ds_no_t2ds_pvalue <= input$pvalue_threshold] = '#CAB2D6' # purple: Pre_T2D vs No_T2D sig
    vertex_col[V(net)$t2ds_no_t2ds_pvalue <= input$pvalue_threshold &
               V(net)$pret2ds_no_t2ds_pvalue <= input$pvalue_threshold] = '#FB9A99' # pink: both sig

    # vertex size: p-value driven
    vertex_size = rep(input$gene_node_size, vcount(net))
    vertex_size[V(net)$t2ds_no_t2ds_pvalue <= input$pvalue_threshold] = input$imgene_node_size
    vertex_size[V(net)$pret2ds_no_t2ds_pvalue <= input$pvalue_threshold] = input$imgene_node_size
    vertex_size[V(net)$t2ds_no_t2ds_pvalue <= input$pvalue_threshold &
                V(net)$pret2ds_no_t2ds_pvalue <= input$pvalue_threshold] = input$imgene_node_size

    # vertex label size: p-value driven
    vertex_cex = rep(input$gene_label_size, vcount(net))
    vertex_cex[V(net)$t2ds_no_t2ds_pvalue <= input$pvalue_threshold] = input$imgene_label_size
    vertex_cex[V(net)$pret2ds_no_t2ds_pvalue <= input$pvalue_threshold] = input$imgene_label_size
    vertex_cex[V(net)$t2ds_no_t2ds_pvalue <= input$pvalue_threshold &
               V(net)$pret2ds_no_t2ds_pvalue <= input$pvalue_threshold] = input$imgene_label_size
    ...
    above_thresh = edge_weight_vals >= input$marking_edge_threshold
