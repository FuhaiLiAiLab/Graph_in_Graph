...
def build_model(num_gene_node, device):
    ...
    这些参数严格对应 geo_analysis_tran_new1.py arg_parse() 中的默认值，
    即产出旧参数对应结果的那组参数。
    """
    model = GIG_Transformer(
        gene_input_dim=6,                       # gene embedding: 输入维度
        gene_hidden_dim=18,                     # gene embedding: 隐藏层维度
        gene_embedding_dim=18,                  # gene_output_dim
        gene_num_top_feature=18,                # 顶部基因特征数
        num_gene_node=num_gene_node,
        gig_input_dim=42,                       # GiG embedding: 输入维度
        gig_input_transform_dim=18,             # GiG 输入变换维度
        gig_hidden_dim=18,                      # GiG 隐藏层维度
        gig_embedding_dim=18,                   # gig_output_dim
        num_classes=3,
        gene_num_head=1,
        gig_num_head=1,
        class_weight_fine=0.7746597442093394,   # 损失函数参数
        class_weight=0.8209978466887934,
        ortho_weight=0.007968476155213464,
        link_weight=0.002195183531312866,
        ent_weight=0.09039903257209198,
        graph_opt='gene'
    )
...
def extract_patient_attentions(fold_n, gene_feature, gene_edge_index,
    ...
    """
    # 加载模型（路径与 geo_analysis_tran_new1.py 的 analysis_model() 一致）
    fold_to_unit = {1: 1, 2: 4, 3: 2, 4: 2, 5: 2}
    model_path = f'./gnn_result/gigtransformer_gene/epoch_500_fold{fold_n}-{fold_to_unit[fold_n]}/best_train_model.pth'
...
def compute_group_statistics(patient_alphas, groups, layer_name='last_alpha'):
    ...
    # ---- 6. 逐边 Wilcoxon 检验 (pret2ds vs t2ds) ----
    if 'pret2ds' in group_all_alphas and 't2ds' in group_all_alphas:
        pret2ds_alphas = group_all_alphas['pret2ds']
        t2ds_alphas_pt = group_all_alphas['t2ds']
        num_edges = pret2ds_alphas.shape[1]

        pvalues_pt = np.ones(num_edges)
        significant_count_pt = 0

        print(f'  正在计算 pret2ds vs t2ds 的 {num_edges} 条边 Wilcoxon 检验 ...')
        for edge_i in range(num_edges):
            pre_vals = pret2ds_alphas[:, edge_i]
            t2_vals = t2ds_alphas_pt[:, edge_i]
            try:
                _, pval = stats.mannwhitneyu(pre_vals, t2_vals, alternative='two-sided')
                pvalues_pt[edge_i] = pval
                if pval < 0.05:
                    significant_count_pt += 1
            except ValueError:
                pvalues_pt[edge_i] = 1.0

        results['pret2ds_vs_t2ds_significant_edges_005'] = int(significant_count_pt)
        results['pret2ds_vs_t2ds_significant_ratio'] = float(significant_count_pt / num_edges)
        bonferroni_threshold_pt = 0.05 / num_edges
        bonferroni_count_pt = int(np.sum(pvalues_pt < bonferroni_threshold_pt))
        results['pret2ds_vs_t2ds_significant_edges_bonferroni'] = bonferroni_count_pt
        print(f'  pret2ds vs t2ds: {significant_count_pt}/{num_edges} 条边 p<0.05 '
              f'({significant_count_pt/num_edges*100:.2f}%)')
        print(f'  Bonferroni 校正后: {bonferroni_count_pt}/{num_edges} 条边显著')
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='verify_output_gene',
                        help='输出目录 (默认: verify_output_gene)')
    ...
    for fold_key, fold_data in all_fold_results.items():
        ...
        print(f'  显著边 (pret2ds vs t2ds, p<0.05): '
              f'{last.get("pret2ds_vs_t2ds_significant_edges_005", "N/A")}/{last.get("total_edges", "N/A")} '
              f'= {last.get("pret2ds_vs_t2ds_significant_ratio", "N/A")} '
              f'[Bonferroni: {last.get("pret2ds_vs_t2ds_significant_edges_bonferroni", "N/A")}]')
