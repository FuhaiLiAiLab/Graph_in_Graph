...
def build_model(num_gene_node, device):
    ...
    model = GIG_Transformer(
        ...
        gene_hidden_dim=9,                      # gene embedding: 隐藏层维度
        gene_embedding_dim=9,                   # gene_output_dim
        gene_num_top_feature=24,                # 顶部基因特征数
        num_gene_node=num_gene_node,
        gig_input_dim=42,                       # GiG embedding: 输入维度
        gig_input_transform_dim=24,             # GiG 输入变换维度
        gig_hidden_dim=24,                      # GiG 隐藏层维度
        gig_embedding_dim=24,                   # gig_output_dim
        num_classes=3,
        gene_num_head=1,
        gig_num_head=1,
...
def extract_patient_attentions(fold_n, gene_feature, gene_edge_index,
    ...
    fold_to_unit = {1: 31, 2: 24, 3: 24, 4: 23, 5: 21}
    model_path = f'./gnn_result/gigtransformer_GinG/epoch_500_fold{fold_n}-{fold_to_unit[fold_n]}/best_train_model.pth'
