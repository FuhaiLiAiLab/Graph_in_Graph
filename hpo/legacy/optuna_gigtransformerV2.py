...
策略：初筛 → 细筛 → Top 3 验证
  Phase 1 (初筛): 3-fold, 100 epochs — 150 trials + MedianPruner, 宽搜索空间
  Phase 2 (细筛): 4-fold, 200 epochs — 50 trials, 缩窄空间
  Phase 3 (最终验证): 5-fold × 5-train, 500 epochs — Top 3
...
用法:
  python optuna_gigtransformerV2.py --phase 1 --n_trials 150   # 3-fold 初筛
  python optuna_gigtransformerV2.py --phase 2 --n_trials 50    # 4-fold 细筛
  python optuna_gigtransformerV2.py --phase 3                  # 5-fold×5-run 最终验证
  python optuna_gigtransformerV2.py --analyze                  # 汇总分析
"""
...
# 配置
# ============================================================
DB_PATH = "optuna_gigtransformer_v2.db"
STUDY_PREFIX = "gigtransformer_v2"
LOG_DIR = "./hpo_logs"
RESULT_DIR = "./hpo_results"

# 新添加的
STABILITY_LAMBDA = 0.3   # score = mean_acc - λ * std_acc
...
PHASE1_SPACE = {
    # === 学习率 ===
    "lr": {"type": "loguniform", "low": 1e-4, "high": 0.05},

    # === Weight decay ===
    "weight_decay": {"type": "loguniform", "low": 1e-10, "high": 0.05},

    # === 学习率衰减 ===
    "gamma": {"type": "categorical", "choices": [0.8, 0.85, 0.9, 0.95, 0.98]},
    "milestone_step": {"type": "categorical", "choices": [50, 100, 150]},

    # === Gene-level 网络结构 ===
    # gene_hidden_dim = gene_output_dim (保持一致)
    "gene_hidden_dim": {"type": "categorical", "choices": [4, 6, 9, 12, 18, 24]},
    # K: assignment matrix 行数
    "gene_num_top_feature": {"type": "categorical", "choices": [6, 8, 12, 16, 18, 24, 32]},

    # === GiG-level 网络结构 ===
    "gig_input_transform_dim": {"type": "categorical", "choices": [12, 18, 24, 36, 42, 48]},
    # gig_hidden_dim = gig_output_dim (保持一致)
    "gig_hidden_dim": {"type": "categorical", "choices": [6, 12, 18, 24, 36, 48]},
    ...
    "clip": {"type": "categorical", "choices": [1.0, 2.0, 5.0, 10.0, 20.0]},
}

# 用于 Phase 2 收窄时的原始边界
ORIG_BOUNDS = {
    "lr": (1e-4, 0.05), "weight_decay": (1e-10, 0.05),
    "class_weight": (0.5, 1.0), "class_weight_fine": (0.1, 1.0),
    "ortho_weight": (1e-3, 0.15), "link_weight": (1e-3, 0.15),
    "ent_weight": (0.0, 0.1), "dropout": (0.0, 0.5),
}
...
def setup_logging(phase):
    ...
    log_file = os.path.join(LOG_DIR, f"transformer_v2_phase{phase}_{timestamp}.log")
...
def make_default_args(**overrides):
    """创建默认参数，和 optuna_gigtransformerV2.py 的 arg_parse() 一致。"""
    defaults = dict(
...
def make_objective(phase, space, num_epochs, eval_folds):
    ...
    def objective(trial):
        ...
        score = mean_acc - STABILITY_LAMBDA * std_acc

        trial.set_user_attr("mean_acc", float(mean_acc))
        trial.set_user_attr("std_acc", float(std_acc))
        trial.set_user_attr("fold_accs", [float(a) for a in accuracies])

        logger.info(f"Trial {trial.number}: mean={mean_acc:.4f}, std={std_acc:.4f}, score={score:.4f}")
        return score
...
def run_phase1(n_trials=150):
    """Phase 1: 3-fold, 100 epochs, 150 trials + MedianPruner, 宽搜索空间"""
    import optuna

    logger = setup_logging(1)
    logger.info("=" * 60)
    logger.info("PHASE 1: 初筛 32-fold, 100 epochs, MedianPruner)")
...
def run_phase2(n_trials=50):
    """Phase 2: 4-fold, 200 epochs, 30 trials, 缩窄空间"""
    import optuna

    logger = setup_logging(2)
    logger.info("=" * 60)
    logger.info("PHASE 2: 细筛 (4-fold, 200 epochs)")
    logger.info(f"Trials: {n_trials}")
    logger.info("=" * 60)
    ...
    narrowed_space = narrow_space_from_study(phase1_study, top_k=15, min_acc=0.20)
...
def run_phase3():
    ...
    for rank, trial in enumerate(top_trials):
        ...
        for fold_n in range(1, 6):
            ...
            fold_results[f"fold_{fold_n}"] = {
                ...
                "best": float(max(fold_accs)) if fold_accs else 0.0,
                "mean": fold_mean,
                "std": float(np.std(fold_accs)) if fold_accs else 0.0,
            }
            logger.info(f"  Fold {fold_n} mean: {fold_mean:.4f}, std: {np.std(fold_accs):.4f}")
        ...
        overall_score = overall_mean - STABILITY_LAMBDA * overall_std
        ...
        logger.info(f"  overall: mean={overall_mean:.4f} +/- {overall_std:.4f}, score={overall_score:.4f}")
    ...
    logger.info(f"{'='*60}")
    for r in results:
        logger.info(f"\n  Rank #{r['rank']}: score={r['overall_score']:.4f}  "
                     f"mean={r['overall_mean']*100:.2f}% +/- {r['overall_std']*100:.2f}%  "
                     f"({r['source']} Trial #{r['source_trial']})")
        logger.info(f"  全部参数:")
        ...
        for fold_key in sorted(r['fold_results'].keys()):
            ...
            logger.info(f"    {fold_key}: mean={fr['mean']:.4f}, std={fr['std']:.4f}, runs={fr['all_accs']}")
    logger.info(f"\n结果已保存到: {result_file}")
    ...
    for r in results:
        ...
        for fold_key in sorted(r['fold_results'].keys()):
            fr = r['fold_results'][fold_key]
            print(f"    {fold_key}: mean={fr['mean']:.4f}, std={fr['std']:.4f}, runs={fr['all_accs']}")
    best = max(results, key=lambda x: x['overall_score'])
    print(f"\nBEST: Rank #{best['rank']}  score={best['overall_score']:.4f}  "
...
def print_all_trials_table(study, phase):
    ...
    header = (f"{'Rank':>4} {'Trial':>5} {'Score':>7} {'Mean':>6} {'Std':>5} | {'lr':>9} {'drop':>5} "
    ...
    for rank, t in enumerate(completed):
        ...
        line = (
            f"{rank+1:>4} "
            f"T{t.number:>4} "
            f"{t.value:.4f} {mean_acc:.4f} {std_acc:.3f} | "
            f"{p.get('lr', 0):.6f} "
    ...
    print(f"{'='*90}")
    print(f"{'Rank':>4} {'Trial':>5} {'Score':>7} {'Mean':>6} {'Std':>5} | {'lr':>9} {'drop':>5} {'g_hid':>5} {'K':>3} {'gig_in':>6} {'gig_h':>5}")
    print("-" * 90)
    for rank, t in enumerate(completed):
        p = t.params
        mean_acc = t.user_attrs.get("mean_acc", t.value)
...
def analyze_results():
    ...
    if os.path.exists(RESULT_DIR):
        ...
        if phase3_files:
            ...
            print(f"{'='*60}")
            for r in results:
                print(f"  Rank #{r['rank']}: score={r.get('overall_score', 'N/A')}  "
                      f"mean={r['overall_mean']*100:.2f}% +/- {r['overall_std']*100:.2f}%")
                print(f"    全部参数:")
                ...
                for fold_key in sorted(r.get('fold_results', {}).keys()):
                    fr = r['fold_results'][fold_key]
                    print(f"      {fold_key}: mean={fr['mean']:.4f}, std={fr.get('std', 0):.4f}, runs={fr.get('all_accs', [])}")
